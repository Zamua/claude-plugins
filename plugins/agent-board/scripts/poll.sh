...
_write_launcher() {  # -> launcher path
  local launcher base
  launcher="$HOME/.config/agent-board/run-poll.sh"
  mkdir -p "$(dirname "$launcher")"
  case "$HERE" in
    */agent-board/*/scripts) base="$(cd "$HERE/../.." && pwd)" ;;
    *)                       base="" ;;
  esac
  {
...
cmd_install() {
  local interval log launcher; interval="$(ab_get poll_seconds 90)"; log="$HOME/.config/agent-board/poll.log"
  mkdir -p "$HOME/.config/agent-board"
  launcher="$(_write_launcher)"
  ab_log "launcher: $launcher (resolves the newest installed version per tick)"
...
cmd_uninstall() {
  if [ "$(uname)" = "Darwin" ]; then
    ...
    rm -f "$HOME/.config/agent-board/run-poll.sh"
    ab_log "removed launchd job and its launcher"
