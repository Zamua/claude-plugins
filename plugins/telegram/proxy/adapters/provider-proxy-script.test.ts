...
describe('provider proxy launcher', () => {
  ...
  test('prefers the nix-darwin per-user bridge and preserves an explicit override', () => {
    expect(script).toContain('TELEGRAM_PROVIDER_PROXY_BIN')
    expect(script).toContain('/etc/profiles/per-user/$USER/bin/claude-code-proxy')
    expect(script).not.toContain('.local/state/nix/profiles/home-manager')
