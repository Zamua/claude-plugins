...
describe('provider-aware inbound delivery', () => {
  test('uses native Claude Channels only for Anthropic', () => {
    expect(inboundModeForRoute({ provider: 'anthropic', model: 'fable', effort: 'xhigh', ultracode: false })).toBe('channel')
    expect(inboundModeForRoute({ provider: 'codex', model: 'gpt-5.6-sol', effort: 'xhigh', ultracode: false })).toBe('pane')
    expect(inboundModeForRoute({ provider: 'opencode-go', model: 'opencode-go/glm-5.2', effort: 'high', ultracode: false })).toBe('pane')
  })
